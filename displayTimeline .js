...
const executionOfCode = () => {
    ...
    let yearOption = [];
    for (let i = 2020; i < thisYear + 2; i++) {
        yearOption.push(i.toString(10));
    }
    yearOption.push("Selectionner l'année");
    ...
    $('select').on('change', function () {
        resetAll();
        if (this.value > 2019 && this.value < 2050) {
            dataRecovery(Number(this.value));
        }
